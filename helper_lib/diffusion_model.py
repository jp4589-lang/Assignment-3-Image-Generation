...
class UNetTiny(nn.Module):
    """Predicts noise ε given noisy image x_t and time t."""
    def __init__(self, img_channels=1, base=32, tdim=64):
        super().__init__()
        self.t_mlp = nn.Sequential(SinusoidalEmbedding(tdim), nn.Linear(tdim, tdim), nn.SiLU())

        self.enc1 = ConvBlock(img_channels, base)         # 1 -> 32
        self.down1 = nn.Conv2d(base, base*2, 4, 2, 1)     # 28 -> 14
        self.enc2 = ConvBlock(base*2, base*2)             # 64

        self.mid  = ConvBlock(base*2, base*2)

        self.up1  = nn.ConvTranspose2d(base*2, base, 4, 2, 1) # 14 -> 28
    ...
    def forward(self, x, t):
        temb = self.t_mlp(t)                      # [B, tdim]
        b = x

        e1 = self.enc1(b)                         # [B,32,28,28]
        d1 = self.down1(e1)                       # [B,64,14,14]
        e2 = self.enc2(d1)                        # [B,64,14,14]
        # inject time at mid
        h  = self.mid(e2 + self.t_to_c1(temb).view(-1, e2.size(1), 1, 1))
        u1 = self.up1(h)                          # [B,32,28,28]
        cat = torch.cat([u1, e1 + self.t_to_c2(temb).view(-1, u1.size(1), 1, 1)], dim=1)
        dec = self.dec1(cat)
        return self.out(dec)                      # predicted ε

def build_diffusion_model(img_channels=1):
